...
public class TreeSet<T extends Comparable<T> > implements Set<T>, Serializable {
    ...
    public void writeTo(Serializer s) throws IOException {
       writeToHelper(root, s);
    }
    private void writeToHelper(Node<T> parent, Serializer s) throws IOException {
        if(parent == null){
            return;
        }
        s.serialize((Serializable) parent.element);
        size++;
        writeToHelper(parent.left,s);
        writeToHelper(parent.right,s);
    }
    ...
    public void readFrom(Serializer s) throws IOException, SerializationException {
        ...
        for(int i =0; i < size;i++){
            if(root == null){
                root = new Node<>((T)s.deserialize());
            }
            else{
                add(root,(T) s.deserialize());
            }
        }
    }
