...
public class HashMap<K,V> implements Map<K,V>, Serializable {
    ...
    public void writeTo(Serializer s) throws IOException {
        ...
        MapIterator yikes = new MapIterator();

        while(yikes.hasNext()){
            Entry ent = yikes.next();
            s.serialize((Serializable)ent.getKey());
            s.serialize((Serializable)ent.getValue());
        }
    ...
    public void readFrom(Serializer s) throws IOException, SerializationException {
        ...
        s.read(bytes);
        size=buffer.getInt();
        HashMap yikes = new HashMap();
        for(int i= 0; i<size;i++){
            if(i % 2 == 0){
                Serializable key=s.deserialize();
                Serializable input=s.deserialize();
                yikes.put(key,input);
                Entry ent = new Entry<>(key,input);
            }
        }
    }
